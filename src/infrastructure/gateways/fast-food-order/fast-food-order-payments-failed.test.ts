...
describe('FastFoodOrderPaymentsFailed', () => {
    ...
    let gateway: FastFoodOrderPaymentsFailed;

    beforeEach(() => {
        vi.clearAllMocks();
        loggerMock = createLoggerMock();
        httpClientMock = createHttpClientMock();
        gateway = new FastFoodOrderPaymentsFailed(loggerMock, httpClientMock);
    });

    it('should update order payments failed status successfully', async () => {
        vi.spyOn(httpClientMock, 'post').mockResolvedValueOnce(undefined);

        await gateway.execute('order-123');

        expect(httpClientMock.post).toHaveBeenCalledWith('http://localhost:3000/order/order-123/failed', {});
        expect(loggerMock.info).toHaveBeenCalledWith(
            'Order payments failed status updated successfully in fast-food-order service',
            { orderId: 'order-123' },
        );
    });
